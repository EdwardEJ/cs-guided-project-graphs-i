...
def numIslands(grid):
	...
	When we see a 1 that we havent seen before
	Increment our counter
	Figure out the boundaries of this island by checking all 4 cardinal directions (up, down, left, right)
	We'll need some way to keep track of which 1s we've visited before to avoid double counting

	If we see a one that we have visited before, dont do anything with that

	How do we want to keep track of which 1s we've visited before?
		- Keeping some variable that denotes whether we're still on land
		- Changing any 1s we've visited before

	How does the "radiating out" process work?
